...
public class EmailVerificationServiceImpl implements EmailVerificationService {
    ...
    @Override
    public boolean verify(Integer id, String emailVerificationToken) {
        var user = userService.findByIdAndEmailVerificationToken(id, emailVerificationToken);
        var current_user = userService.getUser();
        if (!Objects.equals(current_user.getId(), user.getId())) {
            return false;
        } else {
            user.setStatus(Status.VERIFIED);
            userService.save(user);
            var curren_otp = emailVerificationTokenService.findByToken(emailVerificationToken);
            curren_otp.setExpired(1);
            emailVerificationTokenService.save(curren_otp);
            return true;
        }
    }

    @Override
    public boolean sendVerificationEmail(Integer id, String address, String fullName, String otp) {
                                          ...
                                          <tr>
                                            <td style="overflow-wrap:break-word;word-break:break-word;padding:55px;font-family:'Cabin',sans-serif;" align="left">
                                              ...
                                              <div style="color: #ffffff; line-height: 140%; text-align: left; word-wrap: break-word;">
                                                ...
                                                <p style="font-size: 14px; line-height: 140%;"><span style="color: #000000; line-height: 19.6px;">OTP for verification is <font size="+3"><b>[[otp]]</b></font></span><br /><br /><span style="color: #000000; line-height: 19.6px;">Note: This OTP is valid for next 10 mins.</span></p>
    ...
    public boolean resendVerificationEmail() {
        ...
        if (user == null) {
            ...
            emailVerificationTokenService.revokeAllEmailVerificationToken(user.getId());
            var new_otp = generateEmailVerificationToken(user);
            ...
            return sendVerificationEmail(user.getId(), user.getEmail(), user.getFullName(), new_otp);
        }
    }
}
